...
namespace vidrevolt {
    LuaController::LuaController(const std::string& path, bool shared) : shared_(shared), path_(path) {}
    ...
    lua_State* LuaController::getLua(const std::string& control_name) {
        lua_State* L = nullptr;
        if (shared_) {
            L = L_;
        } else {
    ...
    void LuaController::addControl(const LuaController::Control& ctrl) {
    ...
    void LuaController::setControlArgs(const std::string& func_name, std::vector<vidrevolt::Value> args) {
        control_args_[func_name] = args;
    }

    void LuaController::beforePoll() {
    ...
    const std::map<std::string, LuaController::Control>& LuaController::getControls() const {
        return controls_;
    }
}
